...
pub trait DbusWrite {
    fn write<T1, T2>(&self, writer: &mut DbusWriter<T1>) -> Result<u64>
    where
        T1: io::Write,
        T2: ByteOrder;
}
...
impl<T: io::Write> DbusWriter<T> {
    ...
    pub fn write_padding(&mut self, bytes_written: u64) -> Result<u8> {
        let padding_length = (8 - (bytes_written % 8)) % 8;
    ...
    pub fn write_string<T1: ByteOrder>(&mut self, s: &str) -> Result<u64> {
        let mut bytes_written = 0;
        bytes_written += self.write_u32::<T1>(s.len() as u32)?;
    ...
    pub fn write_object_path<T1: ByteOrder>(&mut self, object_path: ObjectPath) -> Result<u64> {
    ...
    pub fn write_signature<T1: ByteOrder>(&mut self, signature: Signature) -> Result<u64> {
    ...
    pub fn write_array<T1: ByteOrder, T2: DbusWrite>(&mut self, a: &[T2]) -> Result<u64> {
        let mut bytes_written = 0;

        bytes_written += self.write_u32::<T1>(a.len() as u32)?;

        for x in a {
            bytes_written += x.write::<_, T1>(self)?;
        }
